...
async function getBalancePolygon_USDC(req, res) {
  try {
    ...
    console.log('Polygon_USDC', resultInEther);
    res.json({
      success: true,
      message: 'Fetched Polygon_USDC balance Successfully',
      body: { resultInEther },
    });
  } catch (error) {
    console.log('failed to get Polygon_USDC balance ', error);
  }
}
